    ...
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 140 }}>
        <View style={styles.heroWrap}>
          ...
          <LinearGradient
            colors={["rgba(10,10,11,0.4)", "rgba(10,10,11,0.85)", c.bg] as any}
          ...
          <View style={styles.heroInner}>
            ...
            <View style={styles.statsRow}>
              <Stat icon={<Users color={c.secondary} size={14} />} label="Kuzatuvchilar" value={formatK(author.followers)} c={c} />
              <View style={styles.statDiv} />
              <Stat icon={<BookOpen color={c.secondary} size={14} />} label="O'qilgan" value={formatK(author.reads)} c={c} />
              <View style={styles.statDiv} />
              <Stat label="Asarlar" value={works.length.toString()} c={c} />
            </View>
...
function Stat({ icon, label, value, c }: { icon?: React.ReactNode; label: string; value: string; c: AppTheme }) {
  return (
    <View style={{ alignItems: "center", flex: 1 }}>
      {icon}
      <Text style={{ color: c.text, fontSize: 16, fontWeight: "700", marginTop: 4 }}>{value}</Text>
...
function createStyles(c: AppTheme) {
  return StyleSheet.create({
    ...
    role: { color: c.secondary, fontSize: 13, marginTop: 6 },
    statsRow: {
      flexDirection: "row",
      alignItems: "center",
      marginTop: 22,
    ...
    workCover: { width: "100%", aspectRatio: 2 / 3, borderRadius: 12, backgroundColor: c.bgCard },
    workTitle: { color: c.text, fontSize: 14, fontWeight: "700", marginTop: 8 },
    workCat: { color: c.secondary, fontSize: 11, marginTop: 2, fontWeight: "600" },
  });
}
