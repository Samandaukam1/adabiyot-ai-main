...
const ENABLE_EXTRA_CURL_SHADOWS = false;
...
const BookPagePortrait = React.forwardRef<PortraitBookInstance, IBookPageProps>(
    (
        ...
        // Vendored patch: Reanimated 4 removed `useAnimatedGestureHandler` and the
        // legacy `PanGestureHandler`. Migrated to the modern `Gesture.Pan()` API.
        // The `ctx` object is replaced by a shared value (`ctxX`); onActive → onUpdate.
        ...
        const panGesture = Gesture.Pan()
            ...
            .onUpdate((event) => {
                'worklet';
                const newX = ctxX.value + event.translationX;
            ...
            .onEnd((event) => {
                ...
                if (rotateYAsDeg.value === degrees) {
                    ...
                    const duration = clamp(
                        360 * progress - Math.abs(0.1 * event.velocityX),
                        160,
                        420
                    );
                    rotateYAsDeg.value = withTiming(
        ...
        const tapGesture = Gesture.Tap()
            .enabled(gesturesEnabled && typeof onPageTap === 'function')
            .maxDuration(400)
            .maxDistance(16)
            .onEnd((_event, success) => {
                'worklet';
                if (success && onPageTap && typeof onPageTap === 'function') {
                    runOnJS(onPageTap)();
                }
            });

        const pageGesture = onPageTap
...
BookPagePortrait.displayName = 'BookPagePortrait';
